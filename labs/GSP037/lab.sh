...
if [ -n "$EXISTING_KEY_NAME" ]; then
  export API_KEY=$(gcloud services api-keys get-key-string "$EXISTING_KEY_NAME" --format="value(keyString)")
else
  gcloud services api-keys create \
    --display-name="$API_KEY_NAME" \
    --api-target="service=vision.googleapis.com" >/dev/null

  CREATED_KEY_NAME=$(gcloud services api-keys list \
    --format="value(name)" \
    --filter="displayName=${API_KEY_NAME}" \
    --limit=1)

  export API_KEY=$(gcloud services api-keys get-key-string "$CREATED_KEY_NAME" --format="value(keyString)")
fi
