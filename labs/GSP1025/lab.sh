...
# Step 2: Set Default Compute Zone
echo "${CYAN}${BOLD}Fetching Default Compute Zone${RESET}"
export ZONE=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-zone])")

# Step 3: Create Kubernetes Cluster
echo "${YELLOW}${BOLD}Creating Kubernetes Cluster with 3 Nodes${RESET}"
gcloud container clusters create gmp-cluster --num-nodes=3 --zone=$ZONE

# Step 4: Retrieve Cluster Credentials
echo "${GREEN}${BOLD}Retrieving Cluster Credentials${RESET}"
gcloud container clusters get-credentials gmp-cluster --zone=$ZONE

# Step 5: Create Namespace
echo "${CYAN}${BOLD}Creating Namespace 'gmp-test'${RESET}"
kubectl create ns gmp-test

# Step 6: Deploy Example Application
echo "${YELLOW}${BOLD}Deploying Example Application${RESET}"
...
# Step 7: Deploy Prometheus Configuration
echo "${GREEN}${BOLD}Deploying Prometheus Configuration${RESET}"
...
echo "${CYAN}${BOLD}Waiting for Pods to Initialize (150s)${RESET}"
sleep 150

# Step 8: Check Pod Status
echo "${YELLOW}${BOLD}Checking Pod Status${RESET}"
kubectl -n gmp-test get pod

# Step 9: Deploy Frontend Application with PROJECT_ID
echo "${GREEN}${BOLD}Deploying Frontend Application${RESET}"
...
# Step 10: Port-Forward Frontend Service
echo "${CYAN}${BOLD}Port-Forwarding Frontend Service on Port 9090${RESET}"
kubectl -n gmp-test port-forward svc/frontend 9090

# Step 11: Clone Prometheus Operator Repository
echo "${GREEN}${BOLD}Cloning Prometheus Operator Repository${RESET}"
git clone https://github.com/prometheus-operator/kube-prometheus.git

# Step 12: Apply Grafana Configuration
echo "${YELLOW}${BOLD}Applying Grafana Configuration${RESET}"
cd kube-prometheus
