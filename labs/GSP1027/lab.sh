...
echo "${GREEN}Setting Project ID and Zone...${RESET}"
export PROJECT=$(gcloud config get-value project)
export ZONE=$(gcloud compute project-info describe \
--format="value(commonInstanceMetadata.items[google-compute-default-zone])")

# Step 2: Create Kubernetes Cluster
echo "${BLUE}Creating Kubernetes Cluster with Managed Prometheus...${RESET}"
...
echo "${MAGENTA}Retrieving Cluster Credentials...${RESET}"
gcloud container clusters get-credentials gmp-cluster --zone=$ZONE

# Step 4: Apply Monitoring Configurations
echo "${CYAN}Applying Self Pod Monitoring Configuration...${RESET}"
...
# Step 5: Applying Example Application
echo "${YELLOW}Applying Example Application...${RESET}"
...
# Step 6: Patch Operator Config
echo "${RED}Patching Operator Configuration...${RESET}"
...
# Step 7: Create Operator Config YAML
echo "${GREEN}Creating Operator Configuration File...${RESET}"
...
echo "${BLUE}Creating Cloud Storage Bucket and Uploading Configurations...${RESET}"
gsutil mb -p $PROJECT gs://$PROJECT
gsutil cp op-config.yaml gs://$PROJECT
gsutil -m acl set -R -a public-read gs://$PROJECT

# Step 9: Create Pod Monitoring YAML
echo "${MAGENTA}Creating Pod Monitoring Configuration File...${RESET}"
...
echo "${CYAN}Uploading Pod Monitoring Configuration to Cloud Storage...${RESET}"
