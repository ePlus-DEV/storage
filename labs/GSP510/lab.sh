...
read -p "${CYAN}${BOLD}Enter Artifact Registry Repo (e.g., demo-repo): ${RESET}" REPO_NAME
read -p "${CYAN}${BOLD}Enter PodMonitoring interval (default 30s): ${RESET}" INTERVAL
read -p "${CYAN}${BOLD}Enter Service Name (e.g.,helloweb-service-mdax): ${RESET}" SERVICE_NAME
