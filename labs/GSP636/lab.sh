...
# ─── AUTO-FETCH REGION ───────────────────────────────────────────────
# ePlus.DEV : auto-detect region so no manual input needed
...
# ePlus.DEV : derive zones from region
...
# ePlus.DEV: create custom VPC + backend-subnet + proxy-only-subnet
...
# ePlus.DEV: backend subnet 10.1.2.0/24
...
# ePlus.DEV: proxy-only-subnet — reserved for Envoy proxies, DO NOT assign backends here
...
# ePlus.DEV: three rules needed — SSH, GCP health check IPs, proxy-only-subnet range
echo "${CYAN_TEXT}${BOLD_TEXT}[TASK 2] Creating firewall rules... | ${RESET_FORMAT}"

# ePlus.DEV: allow SSH from anywhere to tagged instances
...
# ePlus.DEV: GCP health checker source ranges — required for backend health probes
...
# ePlus.DEV: proxy-only-subnet range — Envoy proxies forward client traffic from this range
...
# ePlus.DEV: template includes Apache + hostname page + all 3 network tags
...
# ePlus.DEV - GSP636 backend startup script
apt-get update
...
echo "Page served from: $vm_hostname | ePlus.DEV" | \
...
# ePlus.DEV: mig-a in zone-b
echo "${CYAN_TEXT}${BOLD_TEXT}[TASK 3] Creating MIG mig-a in ${ZONE_A}... | ePlus.DEV${RESET_FORMAT}"
...
# ePlus.DEV: named port tcp80→80 used by backend service port-name mapping
...
# ePlus.DEV: mig-c in zone-c for zonal redundancy
...
# ─── TASK 5: CLIENT VM ───────────────────────────────────────────────
# ePlus.DEV: client VM must be inside lb-network to reach internal LB VIP
...
# ─── WAIT FOR BACKENDS ───────────────────────────────────────────────
# ePlus.DEV: wait for MIG VMs to boot and Apache startup script to complete
...
# ─── HEALTH CHECK ────────────────────────────────────────────────────
# ePlus.DEV: both mig-a and mig-c must show HEALTHY before testing
...
# ─── SELF CLEANUP ────────────────────────────────────────────────────
# ePlus.DEV: remove this script from terminal after execution
rm -- "$0"