...
on_error() {
  ...
  echo -e "${YELLOW}Resources already created are kept. You can normally run the script again.${RESET}"
...
wait_for_job_state() {
  ...
  while true; do
    ...
    echo -e \
      "  Job state: ${YELLOW}${state:-UNKNOWN}${RESET} | Phase: ${YELLOW}${phase:-UNKNOWN}${RESET}"
...
get_destination_customer_count() {
  ...
  output="$(
    ...
    gcloud sql connect \
      ...
      --quiet \
      2>&1 <<'SQL' || true
SELECT CONCAT('EPLUS_COUNT=', COUNT(*))
...
wait_for_destination_count() {
  ...
  while true; do
    ...
    if [[ "$count" =~ ^[0-9]+$ ]]; then
      ...
      echo -e \
        "  Destination customer count: ${YELLOW}${count}${RESET}"
...
[[ -n "$PROJECT_ID" ]] || \
  fail "Unable to detect the active Google Cloud project."
...
[[ -n "$SOURCE_ZONE" ]] || \
  fail "Source VM ${SOURCE_VM} was not found."
...
[[ -n "$SOURCE_IP" ]] || \
  fail "Unable to detect the source VM internal IP."
...
[[ -n "$NETWORK" ]] || \
  fail "Unable to detect the source VPC network."

# ============================================================================
# AUTO DETECT CLOUD SQL REGION
# ============================================================================
...
[[ -n "$REGION" ]] || \
  fail "Cloud SQL instance ${DEST_INSTANCE} was not found."
...
if [[ "$REGION" != "$EXPECTED_REGION" ]]; then
  warn "Lab expects ${EXPECTED_REGION}; detected ${REGION}. Using detected region."
fi

if [[ "$NETWORK" != "$EXPECTED_NETWORK" ]]; then
  warn "Lab expects ${EXPECTED_NETWORK}; detected ${NETWORK}. Using detected VPC."
fi
...
# ============================================================================
# TASK 2 - SOURCE CONNECTION PROFILE
# ============================================================================
...
info \
  "Current migration job: state=${JOB_STATE:-UNKNOWN}, phase=${JOB_PHASE:-UNKNOWN}"
...
if [[ "$JOB_STATE" == "FAILED" ]]; then
  ...
  fail "Existing migration job is FAILED."
